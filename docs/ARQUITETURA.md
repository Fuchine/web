...
| D3 | **Cache de IA compartilhado e versionado.** Chave: `(subtitle_line_id, tipo, idioma_alvo, prompt_version)`. Na cloud, o cache é global entre usuários; no self-host, é local à instância. O catálogo de vídeos (`videos`) segue o mesmo princípio: não tem escopo de usuário — qualquer vídeo importado por qualquer usuário é visível na biblioteca de todos. É uma decisão declarada, não um default acidental: o catálogo compartilhado é o fosso do produto (cada vídeo processado fica grátis para o próximo usuário). Mitigador antes de multiusuário: hide persistido por usuário (tabela de flags, não `useState`). | Segundo usuário do mesmo vídeo custa zero. `prompt_version` permite invalidação controlada quando os prompts melhorarem. |
